...
export default function ProjectEdit() {
  ...
  const onFileChange = async (e) => {
    ...
    try {
      ...
      const res = await authService.client.post('/upload/', formData)
      ...
      if (!url) throw new Error('La respuesta no incluyó la URL de la imagen.')
  ...
  if (loading)
    return (
      <section className="section-frame py-16">
        <div className="h-48 bg-slate-200/70 animate-pulse rounded-3xl" />
      </section>
    )
      ...
      <div className="section-frame py-8 space-y-6">
        <div className="relative overflow-hidden rounded-3xl bg-white/60 ring-1 ring-slate-200 p-6 sm:p-8">
          ...
          <p className="lead mt-2 max-w-2xl">
            Actualizá título, descripción o imagen del proyecto.{' '}
            <strong>*La meta no es editable.</strong>
          </p>
        </div>

        <form
          onSubmit={onSubmit}
          className="card-surface p-6 sm:p-8 max-w-3xl space-y-5"
        >
          {error && (
          ...
          <div className="pt-2 flex gap-3">
            <Link to="/mis-proyectos" className="btn btn-outline">
              Cancelar
            </Link>
            <button
