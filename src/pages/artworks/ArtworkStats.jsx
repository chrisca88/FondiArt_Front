...
export default function ArtworkStats(){
  ...
  const kpi = useMemo(()=>{
    if(!data) return null
    const sold = data.fractionsTotal - data.fractionsLeft
  ...
  const series = useMemo(()=>{
    if(!data) return []
    const sold = data.fractionsTotal - data.fractionsLeft
    const days = Math.max(6, Math.min(14, Math.round((Date.now() - new Date(data.createdAt)) / 86400000)))
    ...
    if (remaining > 0) arr[arr.length-1].y += remaining
    return arr.reverse()
  }, [data])

  if (loading) return <section className="section-frame py-16"><div className="h-40 rounded-3xl bg-slate-200/70 animate-pulse"/></section>
  if (err) return (
    <section className="section-frame py-16">
      <div className="card-surface p-8 text-center">
        <h3 className="text-xl font-bold">Error</h3>
        <p className="text-slate-600 mt-1">{err}</p>
        <div className="mt-4"><Link to="/mis-obras" className="btn btn-primary">Volver</Link></div>
      </div>
    </section>
  )
  if (!data) return null
    ...
    <section className="min-h-[calc(100vh-4rem)] bg-gradient-to-b from-white to-slate-50">
      <div className="section-frame py-8 space-y-6">
        <div className="relative overflow-hidden rounded-3xl bg-white/60 ring-1 ring-slate-200 p-6 sm:p-8">
          ...
          <p className="lead mt-2 max-w-2xl">Visión general de la performance de la obra en el marketplace.</p>
        </div>
        ...
        <div className="card-surface p-6">
          ...
          <div className="h-3 rounded-full bg-slate-200 overflow-hidden">
            <div className="h-full bg-gradient-to-r from-indigo-500 to-blue-500" style={{ width: `${kpi.pct}%` }} />
          </div>
        </div>
