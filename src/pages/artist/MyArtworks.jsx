      ...
      <div className="section-frame pb-16">
        ...
        {!loading && items.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {items.map(it => <OwnerArtworkCard key={it.id} item={it} onStats={()=>navigate(`/obra/${it.id}/estadisticas`)} />)}
          </div>
        )}
      </div>
    </section>
    ...
    <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white/70">
      ...
      <div className="p-4 space-y-2">
        ...
        <div className="flex gap-2 pt-2">
          <Link to={`/obra/${item.id}`} className="btn btn-outline flex-1">Ver en marketplace</Link>
